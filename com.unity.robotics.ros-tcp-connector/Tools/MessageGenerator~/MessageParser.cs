    ...
    public class MessageParser
    {
        ...
        public MessageParser(List<MessageToken> tokens, string outPath, string rosPackageName, string type, Dictionary<string, string> builtInTypeMapping, Dictionary<string, string> builtInTypesDefaultInitialValues, Dictionary<string, string> numericTypeDeserializationFunctions, Dictionary<string, int> numericTypeByteSize, string className = "", string rosMsgName = "")
        {
            ...
            if (rosMsgName.Equals(""))
            {
                if (Char.IsLower(inFileName[0]))
                    Console.WriteLine("File {0} starts with a lowercase character. Message file names should be PascalCased (see : http://wiki.ros.org/ROS/Patterns/Conventions#line-38)", inFileName);
        ...
        public void Parse()
        {
            ...
            using (StreamWriter writer = new StreamWriter(outFilePath, false))
            {
                ...
                //writer.Write("import Foo.Bar.Baz\n");
                ...
                writer.Write(
                    MsgAutoGenUtilities.ONE_TAB + "[Serializable]\n" +
                    MsgAutoGenUtilities.ONE_TAB + "public class " + className + " : Message\n" +
                    MsgAutoGenUtilities.ONE_TAB + "{\n"
                    );

                // Write ROS package name
                writer.Write(MsgAutoGenUtilities.TWO_TABS + "public const string k_RosMessageName = \"" + rosPackageName + "/" + rosMsgName + "\";\n");
                writer.Write(MsgAutoGenUtilities.TWO_TABS + "public override string RosMessageName => k_RosMessageName;\n\n");
                ...
                if (!constants.Contains(identifier))
                {
                    ...
                    if (arraySizes.ContainsKey(identifier))
                    {
                        ...
                        if (arraySize == 0)
                        {
                            ...
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "offset += Alignment(offset, 4);\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "var " + identifier + "ArrayLength = DeserializeLength(data, offset);\n";
                            ...
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "for(var i = 0; i < " + identifier + "ArrayLength; i++)\n";
                        }
                        else
                        {
                            // fixed length array, we already know the length
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "this." + identifier + "= new " + type + "[" + arraySize + "];\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "for(var i = 0; i < " + arraySize + "; i++)\n";
                        }
                        ...
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.TWO_TABS + "offset += Alignment(offset, 4);\n";
                        ...
                        else if (numericTypeDeserializationFunctions.ContainsKey(type))
                        {
                            ...
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.TWO_TABS + $"offset += Alignment(offset, sizeof({type}));\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.TWO_TABS + $"this.{identifier}[i] = {converterFunction};\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.TWO_TABS + $"offset += sizeof({type});\n";
                        }
                    ...
                    else if (!MsgAutoGenUtilities.builtInTypesMapping.ContainsValue(type) || type.Equals("TimeMsg") || type.Equals("DurationMsg"))
                    {
                    ...
                    else
                    {
                        ...
                        if(offsetAddition > 1) // single bytes never need alignment
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"offset += Alignment(offset,  " + offsetAddition + ");\n";
            ...
            foreach (string identifier in symbolTable.Keys)
            {
                if (!constants.Contains(identifier))
                {
                    ...
                    if (arraySizes.ContainsKey(identifier))
                    {
                        ...
                        if (arraySize == 0)
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset = AddAlignment(curOffset, 4, listOfSerializations);\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"listOfSerializations.Add(BitConverter.GetBytes({identifier}.Length));\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += 4;\n\n";
                        }
                        else
                        {
                        ...
                        if (type == "byte")
                        {
                            ...
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += {identifier}.Length;\n\n";
                        }
                        else if (type == "sbyte")
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"listOfSerializations.Add(Array.ConvertAll(this.{identifier}, a => (byte)a));\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += {identifier}.Length;\n\n";
                        }
                        else if (type.Equals("string"))
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"foreach (var entry in this.{identifier})\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB +  "{\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "     curOffset = AddSerializeString(curOffset, entry, listOfSerializations);\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB +  "}\n";
                        }
                        ...
                        else if (!MsgAutoGenUtilities.builtInTypesMapping.ContainsValue(type))
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"foreach(var entry in this.{identifier})\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "{\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.TWO_TABS + "curOffset = entry.AddSerializationStatements(curOffset, listOfSerializations);\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "}\n";
                        }
                        // Assume numeric type
                        else
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset = AddAlignment(curOffset, sizeof({type}), listOfSerializations);\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"foreach (var entry in this.{identifier})\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB +  "{\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.TWO_TABS + "    listOfSerializations.Add(BitConverter.GetBytes(entry));\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB +  "}\n\n";
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += sizeof({type}) * this.{identifier}.Length;\n";
                        }
                    }
                    // End of Array
                    ...
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset = AddSerializeString(curOffset, this.{identifier}, listOfSerializations);\n";
                    }
                    else if (type.Equals("byte"))
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "listOfSerializations.Add(new byte[]{this." + identifier +"});\n";
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += 1;\n\n";
                    }
                    else if (type.Equals("sbyte"))
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "listOfSerializations.Add(new byte[]{(byte)this." + identifier + "});\n";
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += 1;\n\n";
                    }
                    // Message is a custom type and call it's serialization function
                    else if (!MsgAutoGenUtilities.builtInTypesMapping.ContainsValue(type) || type.Equals("TimeMsg") || type.Equals("DurationMsg"))
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset = this.{identifier}.AddSerializationStatements(curOffset, listOfSerializations);\n";
                    }
                    // Assume numeric type
                    else
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset = AddAlignment(curOffset, sizeof({type}), listOfSerializations);\n";
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"listOfSerializations.Add(BitConverter.GetBytes(this.{identifier}));\n";
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"curOffset += sizeof({type});\n\n";
                    }
                }
            }

            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + "return curOffset;\n";
            function += MsgAutoGenUtilities.TWO_TABS + "}\n\n";
