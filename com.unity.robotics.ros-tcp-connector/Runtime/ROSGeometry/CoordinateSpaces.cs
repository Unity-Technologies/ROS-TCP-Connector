...
namespace Unity.Robotics.ROSTCPConnector.ROSGeometry 
{
    ...
    public static class CoordinateSpaceExtensions
    {
        ...
        public static Vector3<C> As<C>(this RosMessageTypes.Geometry.Point self) where C : ICoordinateSpace, new()
        {
            return new Vector3<C>((float)self.x, (float)self.y, (float)self.z);
        }

        public static Vector3 From<C>(this RosMessageTypes.Geometry.Point self) where C : ICoordinateSpace, new()
        {
            return new Vector3<C>((float)self.x, (float)self.y, (float)self.z).toUnity;
        }

        public static Vector3<C> As<C>(this RosMessageTypes.Geometry.Point32 self) where C : ICoordinateSpace, new()
        {
            return new Vector3<C>(self.x, self.y, self.z);
        }

        public static Vector3 From<C>(this RosMessageTypes.Geometry.Point32 self) where C : ICoordinateSpace, new()
        {
            return new Vector3<C>(self.x, self.y, self.z).toUnity;
        }

        public static Vector3<C> As<C>(this RosMessageTypes.Geometry.Vector3 self) where C : ICoordinateSpace, new()
        {
            return new Vector3<C>((float)self.x, (float)self.y, (float)self.z);
        }

        public static Vector3 From<C>(this RosMessageTypes.Geometry.Vector3 self) where C : ICoordinateSpace, new()
        {
            return new Vector3<C>((float)self.x, (float)self.y, (float)self.z).toUnity;
        }

        public static Quaternion<C> As<C>(this RosMessageTypes.Geometry.Quaternion self) where C : ICoordinateSpace, new()
        {
            return new Quaternion<C>((float)self.x, (float)self.y, (float)self.z, (float)self.w);
        }

        public static Quaternion From<C>(this RosMessageTypes.Geometry.Quaternion self) where C : ICoordinateSpace, new()
        {
            return new Quaternion<C>((float)self.x, (float)self.y, (float)self.z, (float)self.w).toUnity;
        }

        public static RosMessageTypes.Geometry.Transform To<C>(this Transform transform) where C : ICoordinateSpace, new()
        {
