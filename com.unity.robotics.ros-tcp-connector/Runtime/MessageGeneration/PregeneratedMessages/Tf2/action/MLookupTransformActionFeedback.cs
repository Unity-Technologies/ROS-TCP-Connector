using System.Collections.Generic;
using Unity.Robotics.ROSTCPConnector.MessageGeneration;
using RosMessageTypes.Std;
using RosMessageTypes.Actionlib;

namespace RosMessageTypes.Tf2
{
    public class MLookupTransformActionFeedback : ActionFeedback<MLookupTransformFeedback>
    {
        public const string k_RosMessageName = "tf2_msgs/LookupTransformActionFeedback";
        public override string RosMessageName => k_RosMessageName;


        public MLookupTransformActionFeedback() : base()
        {
            this.feedback = new MLookupTransformFeedback();
        }

        public MLookupTransformActionFeedback(MHeader header, MGoalStatus status, MLookupTransformFeedback feedback) : base(header, status)
        {
            this.feedback = feedback;
        }
        public override List<byte[]> SerializationStatements()
        {
            var listOfSerializations = new List<byte[]>();
            listOfSerializations.AddRange(this.header.SerializationStatements());
            listOfSerializations.AddRange(this.status.SerializationStatements());
            listOfSerializations.AddRange(this.feedback.SerializationStatements());

            return listOfSerializations;
        }

        public override int Deserialize(byte[] data, int offset)
        {
            offset = this.header.Deserialize(data, offset);
            offset = this.status.Deserialize(data, offset);
            offset = this.feedback.Deserialize(data, offset);

            return offset;
        }

    }
}
