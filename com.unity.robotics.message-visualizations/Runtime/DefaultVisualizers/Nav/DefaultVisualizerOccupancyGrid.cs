...
public class DefaultVisualizerOccupancyGrid : TexturedDrawingVisualizer<OccupancyGridMsg>
{
    [SerializeField]
    ...
    {
        var state = ROSConnection.GetOrCreateInstance().GetTopic(Topic);

        if (state == null)
            return null;

        RosTopicVisualizationState visTopic = RosTopicVisualizationState.GetOrCreate(state);
        return visTopic.Visual as IDrawingTextureVisual;
    }
    ...
    public override Texture2D CreateTexture(OccupancyGridMsg message, MessageMetadata meta)
    {
        var rotation = message.info.origin.orientation.From<FLU>();
        rotation.eulerAngles += new Vector3(0, -90, 0); // TODO: Account for differing texture origin
