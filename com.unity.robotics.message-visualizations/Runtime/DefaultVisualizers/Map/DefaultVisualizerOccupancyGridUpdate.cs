...
public class DefaultVisualizerOccupancyGridUpdate : DrawingTextureVisualFactory<OccupancyGridUpdateMsg>
{
    [SerializeField]
    ...
    public override void Draw(BasicDrawing drawing, OccupancyGridUpdateMsg message, MessageMetadata meta)
    {
        if (state == null)
        {
            state = ROSConnection.instance.HUDPanel.GetVisualizationState(m_Topic, true);
        }

        if (state.WindowContents != null)
        {
            if (visual == null)
            {
                visual = state.WindowContents.GetVisual() as IDrawingTextureVisual;
            }
    ...
    {
        var state = ROSConnection.instance.HUDPanel.GetVisualizationState(OccupancyGridTopic, true);
        if (state.WindowContents != null)
        {
            var visual = state.WindowContents.GetVisual() as ITextureVisual;
            if (visual != null)
            {
                int width = (int)message.width;
                int height = (int)message.height;
