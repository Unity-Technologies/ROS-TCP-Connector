...
There are a variety of reasons that visualizations can be useful in a simulation. Seeing the data being sent and received within the context of the simulated world offers more information and a better understanding of the state of the simulation, such as seeing realtime sensor data and control signals. Visualizations also enable users to more efficiently debug their robotic simulations with the ability to visually verify coordinate frames, object scaling, offsets, and more.

This page provides you with steps on how to get started with message visualizations in your Unity project, from installing the package to customizing a visualization suite.
...
2. If you have not already added the Message Visualizations package in your Unity project, follow the [Installation steps](../../README.md#Installation).
3. To add the default visualization suite, open the Unity scene that you would like add message visualizations to. In the Project window, expand `Packages/Message Visualizations`. Select the `DefaultVisualizationSuite` (indicated by the blue cube Prefab icon) and drag it into your scene Hierarchy.

    ![](../images~/VizPrefab.png)

4. Enter Play mode. The heads-up display (HUD) panel in the top left corner of the Game view indicates a successful connection via the colored arrows. If the HUD is not visible, ensure your connection throws no errors, and that `Show HUD` in the ROS Settings is on. More information about the HUD can be found [below](#the-hud).
...
5. Once connected, you may begin sending and receiving ROS messages as usual. Topics will by default populate in the HUD panel's `Topics` list. For this example, run a simple publisher from your ROS workspace:
    ...
    ```bash
    # ROS 2
    ros2 topic pub /point geometry_msgs/msg/Point '{x: 1, y: 2, z: 3}’
    ```

    Select the `Topics` tab in the HUD. You will see this sample topic, `/point`, appear in the topics list. Click the topic name to toggle both the `UI` and `Viz` options (alternatively, you can select each individual toggle). `UI` toggles a GUI window that displays a text-formatted version of the message. `Viz` toggles the 3D drawing.
...
6. You should now see a new `UI` window labeled with the topic in your Game view, populated with the `geometry_msgs/Point` data being published. Additionally, the `Viz` is being drawn in the scene (you may need to adjust the Camera to see it in the Game view). You've now successfully set up visualizations!
...
In the [Getting Started](#getting-started) example, the `/point` topic is subscribed to and visualized based on the ROS message type. You can also directly assign a topic in the visualizer component's Inspector--for this example, you can find the `Point Default Visualizer` component by expanding the `DefaultVisualizationSuite` GameObject in the Hierarchy and selecting the `Geometry` child. In the Inspector window, you will see all the default visualizers provided for this package (you may need to scroll down to find `Point`).
...
> The sample screenshot above shows sensor_msgs/PointCloud2 data from the demo [webviz.io rosbag](https://open-source-webviz-ui.s3.amazonaws.com/demo.bag).
